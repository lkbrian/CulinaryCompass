recipes_info = [
     {
        "name": "Samosa Chaat",
        "description": "A popular Kenyan street food snack made with crispy samosas topped with spicy chutneys, yogurt, and crunchy toppings.",
        "ingredients": [
            "10 samosas, store-bought or homemade",
            "1 cup chickpeas, cooked",
            "1 onion, finely chopped",
            "2 tomatoes, chopped",
            "1/2 cup plain yogurt",
            "1/4 cup tamarind chutney",
            "1/4 cup mint-coriander chutney",
            "1/4 cup sev (crunchy chickpea noodles)",
            "1/4 cup chopped cilantro",
            "1/4 cup chopped red onion",
            "1/4 cup chopped tomatoes",
            "1/4 cup chopped green chilies",
            "1 lemon, cut into wedges",
            "Salt, to taste"
        ],
        "instructions": [
            "Prepare Samosas: If using store-bought samosas, reheat them according to package instructions. If using homemade samosas, fry or bake them until crispy.",
            "Assemble Chaat: Arrange the samosas on a serving plate. Top each samosa with cooked chickpeas, finely chopped onion, chopped tomatoes, plain yogurt, tamarind chutney, and mint-coriander chutney.",
            "Add Crunch: Sprinkle sev over the samosas for extra crunch.",
            "Garnish: Garnish the samosa chaat with chopped cilantro, chopped red onion, and chopped green chilies.",
            "Serve: Serve samosa chaat immediately with lemon wedges on the side. Enjoy the explosion of flavors!"
        ],
        "cook_time": "No cooking time specified as it involves assembling pre-cooked samosas and toppings."
    },
    {
        "name": "Bhajia",
        "description": "Deep-fried Kenyan snacks made with seasoned potato slices coated in a chickpea flour batter.",
        "ingredients": [
            "2 large potatoes, thinly sliced",
            "1 cup chickpea flour (besan)",
            "1/2 teaspoon ground turmeric",
            "1 teaspoon chili powder",
            "1 teaspoon ground cumin",
            "1/2 teaspoon baking powder",
            "Salt, to taste",
            "Water, as needed",
            "Oil, for frying"
        ],
        "instructions": [
            "Prepare Batter: In a mixing bowl, combine chickpea flour, ground turmeric, chili powder, ground cumin, baking powder, and salt. Gradually add water while whisking to form a smooth batter.",
            "Coat Potatoes: Dip thinly sliced potatoes into the chickpea flour batter, ensuring they are evenly coated.",
            "Fry Bhajias: Heat oil in a deep fryer or heavy-bottomed pot to 180°C (350°F). Fry the coated potato slices in batches until golden brown and crispy, about 3-4 minutes per batch. Drain on paper towels.",
            "Serve: Serve hot bhajias with chutney or sauce of your choice. Enjoy these crunchy and flavorful snacks!"
        ],
        "cook_time": "About 3-4 minutes per batch for frying the bhajias."
    },
    {
        "name": "Kunde",
        "description": "A traditional Kenyan dish made with cowpeas cooked in coconut milk and flavored with spices.",
        "ingredients": [
            "2 cups cowpeas (black-eyed peas), soaked overnight",
            "2 cups coconut milk",
            "1 onion, finely chopped",
            "2 tomatoes, chopped",
            "2 cloves garlic, minced",
            "1 tablespoon ginger, minced",
            "1 teaspoon ground coriander",
            "1 teaspoon ground cumin",
            "1/2 teaspoon turmeric",
            "1/2 teaspoon paprika",
            "Salt and black pepper, to taste",
            "2 tablespoons vegetable oil",
            "Fresh cilantro, for garnish"
        ],
        "instructions": [
            "Cook Cowpeas: In a large pot, combine soaked cowpeas and enough water to cover them. Bring to a boil, then reduce heat and let it simmer for 30-40 minutes, or until the cowpeas are tender. Drain and set aside.",
            "Sauté Aromatics: In a skillet, heat vegetable oil over medium heat. Add finely chopped onion and sauté until translucent. Stir in minced garlic and minced ginger, cooking until fragrant.",
            "Add Tomatoes: Stir in chopped tomatoes and cook until softened.",
            "Add Coconut Milk: Pour coconut milk into the skillet with the aromatics. Season with ground coriander, ground cumin, turmeric, paprika, salt, and black pepper.",
            "Simmer: Let the mixture simmer for 10-15 minutes, allowing the flavors to meld and the sauce to thicken.",
            "Combine: Add cooked cowpeas to the skillet and stir to coat them in the coconut milk sauce. Cook for an additional 5 minutes.",
            "Serve: Garnish kunde with fresh cilantro before serving. Enjoy this creamy and flavorful dish!"
        ],
        "cook_time": "Approximately 30-40 minutes for simmering the cowpeas, plus additional time for sautéing and simmering the sauce."
    },
    {
        "name": "Ugali with Sukuma Wiki",
        "description": "A staple Kenyan dish made with maize flour served with collard greens.",
        "ingredients": [
            "2 cups maize flour (Unga wa Ugali)",
            "4 cups water",
            "1 bunch sukuma wiki (collard greens), chopped",
            "2 tablespoons vegetable oil",
            "Salt, to taste"
        ],
        "instructions": [
            "Prepare Ugali: In a large saucepan, bring water to a boil. Gradually add maize flour to the boiling water while stirring continuously to avoid lumps. Cook over medium heat, stirring constantly, until the mixture thickens and forms a stiff dough-like consistency.",
            "Shape Ugali: Using a wooden spoon, shape the thickened maize mixture into a round mound in the saucepan. Cover and let it simmer for 5-10 minutes until fully cooked.",
            "Cook Sukuma Wiki: In a separate skillet, heat vegetable oil over medium heat. Add chopped sukuma wiki and sauté until wilted and tender. Season with salt to taste.",
            "Serve: Serve hot Ugali with sukuma wiki on the side. Enjoy!"
        ]
    ,"cook_time": "Approximately 15-20 minutes for cooking the ugali, plus additional time for sautéing the sukuma wiki."
    },
    {
        "name": "Nyama Choma",
        "description": "Grilled meat seasoned with Kenyan spices, typically served with ugali and kachumbari salad.",
        "ingredients": [
            "1 kg beef or goat meat, cut into chunks",
            "2 tablespoons vegetable oil",
            "2 tablespoons soy sauce",
            "2 cloves garlic, minced",
            "1 tablespoon ginger, minced",
            "1 teaspoon paprika",
            "1/2 teaspoon black pepper",
            "Salt, to taste",
            "Lemon wedges, for serving"
        ],
        "instructions": [
            "Marinate Meat: In a large bowl, combine vegetable oil, soy sauce, minced garlic, minced ginger, paprika, black pepper, and salt. Add meat chunks to the marinade and toss to coat evenly. Let it marinate for at least 1 hour or overnight in the refrigerator.",
            "Grill Meat: Preheat the grill to medium-high heat. Thread marinated meat onto skewers and grill for 8-10 minutes, turning occasionally, until charred and cooked to desired doneness.",
            "Serve: Serve Nyama Choma hot off the grill with lemon wedges on the side. Enjoy with ugali and kachumbari salad."
        ],
        "cook_time": "Approximately 8-10 minutes for grilling the meat."
    },
    {
        "name": "Pilau",
        "description": "A fragrant rice dish cooked with meat, spices, and vegetables, popular in Kenyan cuisine.",
        "ingredients": [
            "2 cups basmati rice",
            "500g chicken, beef, or goat meat, diced",
            "1 large onion, finely chopped",
            "3 cloves garlic, minced",
            "1 tablespoon ginger, minced",
            "2 tomatoes, chopped",
            "2 cups beef or chicken broth",
            "1 teaspoon ground cumin",
            "1 teaspoon ground coriander",
            "1 teaspoon ground cinnamon",
            "1 teaspoon turmeric",
            "1/2 teaspoon black pepper",
            "Salt, to taste",
            "2 tablespoons vegetable oil",
            "Fresh coriander leaves, for garnish"
        ],
        "instructions": [
            "Sauté Aromatics: In a large pot, heat vegetable oil over medium heat. Add finely chopped onion and sauté until translucent. Stir in minced garlic and minced ginger, cooking until fragrant.",
            "Cook Meat: Add diced meat to the pot and cook until browned on all sides. Stir in chopped tomatoes and cook until softened.",
            "Add Spices: Sprinkle ground cumin, ground coriander, ground cinnamon, turmeric, black pepper, and salt over the meat mixture. Stir to combine and cook for 2-3 minutes.",
            "Cook Rice: Rinse basmati rice under cold water until the water runs clear. Add rinsed rice to the pot with the meat and spices. Stir to coat the rice with the spice mixture.",
            "Simmer: Pour beef or chicken broth into the pot and bring the mixture to a simmer. Cover and let it cook over low heat for 20-25 minutes, or until the rice is cooked through and the liquid is absorbed.",
            "Serve: Fluff the pilau with a fork and garnish with fresh coriander leaves before serving. Enjoy!"
        ],
        "cook_time": "Approximately 20-25 minutes for cooking the rice and spices."
    },
    {
        "name": "Samosa",
        "description": "Crispy pastry filled with spiced meat or vegetables, commonly served as a snack or appetizer in Kenya.",
        "ingredients": [
            "2 cups all-purpose flour",
            "1/4 cup vegetable oil",
            "1/2 teaspoon salt",
            "1/2 cup water",
            "2 tablespoons vegetable oil",
            "1 onion, finely chopped",
            "2 cloves garlic, minced",
            "1 tablespoon ginger, minced",
            "300g ground beef or potatoes, boiled and mashed",
            "1 teaspoon ground cumin",
            "1 teaspoon ground coriander",
            "1/2 teaspoon turmeric",
            "1/2 teaspoon paprika",
            "Salt and black pepper, to taste",
            "Oil, for frying"
        ],
        "instructions": [
            "Prepare Dough: In a mixing bowl, combine all-purpose flour, vegetable oil, and salt. Gradually add water while kneading until a smooth dough forms. Cover and let it rest for 30 minutes.",
            "Prepare Filling: In a skillet, heat vegetable oil over medium heat. Add finely chopped onion and sauté until translucent. Stir in minced garlic and minced ginger, cooking until fragrant.",
            "Add Meat or Vegetables: If using ground beef, add it to the skillet and cook until browned. If using mashed potatoes, add them to the skillet and stir to combine. Season with ground cumin, ground coriander, turmeric, paprika, salt, and black pepper. Cook for an additional 5 minutes. Remove from heat and let the filling cool.",
            "Shape Samosas: Divide the rested dough into small balls. Roll out each ball into a thin circle. Cut each circle in half to form semi-circles. Spoon a portion of the filling onto one half of each semi-circle. Fold the other half over the filling to form a triangle. Seal the edges tightly.",
            "Fry Samosas: Heat oil in a deep fryer or heavy-bottomed pot to 180°C (350°F). Fry samosas in batches until golden brown and crispy, about 5-7 minutes per batch. Drain on paper towels.",
            "Serve: Serve hot samosas with chutney or sauce of your choice. Enjoy!"
        ],
        "cook_time": "About 5-7 minutes per batch for frying the samosas."
    },
    {
        "name": "Mandazi",
        "description": "Sweet and fluffy East African doughnuts flavored with coconut milk and spices, often enjoyed as a snack or breakfast treat.",
        "ingredients": [
            "2 cups all-purpose flour",
            "1/4 cup sugar",
            "1/2 teaspoon ground cinnamon",
            "1/4 teaspoon ground cardamom",
            "1/4 teaspoon ground cloves",
            "1/4 teaspoon salt",
            "1/4 cup coconut milk",
            "1/4 cup water",
            "2 tablespoons vegetable oil",
            "Oil, for frying"
        ],
        "instructions": [
            "Prepare Dough: In a mixing bowl, combine all-purpose flour, sugar, ground cinnamon, ground cardamom, ground cloves, and salt. Mix well.",
            "Make Dough: Gradually add coconut milk, water, and vegetable oil to the dry ingredients, kneading until a soft dough forms. Cover and let it rest for 30 minutes.",
            "Shape Mandazi: On a floured surface, roll out the rested dough to about 1/4-inch thickness. Use a knife to cut the dough into squares or triangles.",
            "Fry Mandazi: Heat oil in a deep fryer or heavy-bottomed pot to 180°C (350°F). Fry mandazi in batches until golden brown and puffed up, about 2-3 minutes per side. Drain on paper towels.",
            "Serve: Serve warm mandazi as a snack or breakfast treat. Enjoy with tea or coffee."
        ],
        "cook_time": "Approximately 2-3 minutes per batch for frying the mandazi."
    },
    {
        "name": "Githeri",
        "description": "A hearty Kenyan stew made with mixed beans, corn, and assorted vegetables, often served with a side of ugali.",
        "ingredients": [
            "1 cup dried mixed beans (such as kidney beans, black-eyed peas)",
            "2 cups corn kernels",
            "1 onion, finely chopped",
            "2 cloves garlic, minced",
            "1 tablespoon vegetable oil",
            "2 tomatoes, chopped",
            "2 potatoes, peeled and diced",
            "2 cups vegetable broth",
            "1 teaspoon ground cumin",
            "1 teaspoon ground coriander",
            "1/2 teaspoon turmeric",
            "Salt and black pepper, to taste"
        ],
        "instructions": [
            "Soak Beans: Rinse dried mixed beans under cold water and soak them in water overnight. Drain before using.",
            "Cook Beans: In a large pot, heat vegetable oil over medium heat. Add finely chopped onion and minced garlic, sautéing until softened and fragrant.",
            "Add Tomatoes: Stir in chopped tomatoes and cook until softened, about 5 minutes.",
            "Add Beans and Corn: Add soaked mixed beans, corn kernels, diced potatoes, and vegetable broth to the pot. Bring to a boil, then reduce heat and let it simmer for 1-2 hours, or until beans are tender and stew is thickened.",
            "Season: Season githeri with ground cumin, ground coriander, turmeric, salt, and black pepper to taste. Adjust seasoning as needed.",
            "Serve: Serve hot githeri as a hearty stew, accompanied by ugali or bread. Enjoy!"
        ],
        "cook_time": "Approximately 1-2 hours for simmering the stew until beans are tender."
    },
    {
        "name": "Chapati",
        "description": "Soft and flaky flatbread, commonly served as a side dish or used to wrap savory fillings.",
        "ingredients": [
            "2 cups all-purpose flour",
            "1/2 teaspoon salt",
            "1 tablespoon vegetable oil",
            "3/4 cup water",
            "Extra flour, for dusting",
            "Oil or ghee, for frying"
        ],
        "instructions": [
            "Prepare Dough: In a mixing bowl, combine all-purpose flour and salt. Make a well in the center and add vegetable oil and water. Gradually incorporate the flour into the wet ingredients to form a soft dough.",
            "Knead Dough: Transfer the dough to a floured surface and knead for 5-7 minutes until smooth and elastic. Divide the dough into equal-sized balls.",
            "Roll Out Chapatis: Roll out each dough ball into a thin circle, dusting with flour as needed to prevent sticking.",
            "Cook Chapatis: Heat a skillet or griddle over medium-high heat. Place a rolled-out chapati onto the hot skillet and cook for 1-2 minutes on each side, or until lightly golden and puffed up. Brush with oil or ghee while cooking.",
            "Serve: Serve hot chapatis with your favorite Kenyan dishes or use them to wrap savory fillings. Enjoy!"
        ],
        "cook_time": "About 1-2 minutes per side for cooking the chapatis."
    },
    {
        "name": "Mukimo",
        "description": "A traditional Kikuyu dish made with mashed potatoes, green peas, corn, and pumpkin leaves, often served with meat stew.",
        "ingredients": [
            "4 large potatoes, peeled and diced",
            "1 cup green peas, fresh or frozen",
            "1 cup corn kernels",
            "1 cup pumpkin leaves (sukuma wiki), chopped",
            "1 onion, finely chopped",
            "2 cloves garlic, minced",
            "2 tablespoons vegetable oil",
            "Salt and black pepper, to taste"
        ],
        "instructions": [
            "Cook Potatoes: In a large pot of salted boiling water, cook diced potatoes until tender, about 15-20 minutes. Drain and set aside.",
            "Sauté Aromatics: In a skillet, heat vegetable oil over medium heat. Add finely chopped onion and minced garlic, sautéing until softened and fragrant.",
            "Add Vegetables: Stir in green peas, corn kernels, and chopped pumpkin leaves. Cook until the vegetables are tender, about 5-7 minutes.",
            "Mash Potatoes: In a large bowl, mash cooked potatoes until smooth. Add cooked vegetables to the mashed potatoes and mix until well combined. Season with salt and black pepper to taste.",
            "Serve: Serve hot mukimo as a side dish, accompanied by meat stew or your favorite Kenyan dishes. Enjoy!"
        ],
         "cook_time": "Approximately 15-20 minutes for cooking the potatoes and additional time for sautéing the vegetables."
    },
    {
        "name": "Maharagwe",
        "description": "A flavorful Kenyan dish featuring red kidney beans cooked in coconut milk and spices, often served with rice or ugali.",
        "ingredients": [
            "2 cups dried red kidney beans",
            "4 cups water",
            "1 onion, finely chopped",
            "2 cloves garlic, minced",
            "1 tablespoon ginger, minced",
            "2 tomatoes, chopped",
            "1 cup coconut milk",
            "1 teaspoon ground cumin",
            "1 teaspoon ground coriander",
            "1/2 teaspoon turmeric",
            "1/2 teaspoon paprika",
            "Salt and black pepper, to taste",
            "2 tablespoons vegetable oil",
            "Fresh coriander leaves, for garnish"
        ],
        "instructions": [
            "Soak Beans: Rinse dried red kidney beans under cold water and soak them in water overnight. Drain before using.",
            "Cook Beans: In a large pot, combine soaked red kidney beans and water. Bring to a boil, then reduce heat and let it simmer for 1-2 hours, or until beans are tender.",
            "Sauté Aromatics: In a skillet, heat vegetable oil over medium heat. Add finely chopped onion and sauté until translucent. Stir in minced garlic and minced ginger, cooking until fragrant.",
            "Add Tomatoes: Stir in chopped tomatoes and cook until softened, about 5 minutes.",
            "Add Coconut Milk: Pour coconut milk into the skillet with the sautéed aromatics. Stir in ground cumin, ground coriander, turmeric, paprika, salt, and black pepper.",
            "Simmer: Let the mixture simmer for 10-15 minutes, allowing the flavors to meld and the sauce to thicken.",
            "Combine: Add cooked red kidney beans to the skillet and stir to coat them in the coconut milk sauce. Cook for an additional 5 minutes.",
            "Serve: Garnish maharagwe with fresh coriander leaves before serving. Enjoy hot with rice or ugali."
        ],
        "cook_time": "Approximately 1-2 hours for simmering the beans and sauce."
    },
    {
        "name": "Sukuma Wiki",
        "description": "A simple and nutritious Kenyan vegetable dish made with collard greens, often served with ugali or rice.",
        "ingredients": [
            "1 bunch sukuma wiki (collard greens), chopped",
            "1 onion, finely chopped",
            "2 tomatoes, chopped",
            "2 tablespoons vegetable oil",
            "2 cloves garlic, minced",
            "1 teaspoon ground cumin",
            "1 teaspoon ground coriander",
            "Salt and black pepper, to taste"
        ],
        "instructions": [
            "Sauté Aromatics: In a large skillet, heat vegetable oil over medium heat. Add finely chopped onion and sauté until translucent. Stir in minced garlic and cook until fragrant.",
            "Add Tomatoes: Stir in chopped tomatoes and cook until softened, about 5 minutes.",
            "Cook Sukuma Wiki: Add chopped sukuma wiki to the skillet and stir to combine with the sautéed aromatics. Cook until the greens are wilted and tender, about 10-15 minutes.",
            "Season: Sprinkle ground cumin, ground coriander, salt, and black pepper over the sukuma wiki. Stir to evenly distribute the spices.",
            "Serve: Serve hot sukuma wiki as a nutritious side dish, accompanied by ugali or rice"
    ],
           "cook_time": "Approximately 10-15 minutes for cooking the sukuma wiki."
    },
    {
        "name": "Viazi Karai",
        "description": "Spicy Kenyan-style fried potatoes coated in a seasoned chickpea flour batter.",
        "ingredients": [
            "4 potatoes, peeled and cut into wedges",
            "1 cup chickpea flour (besan)",
            "1 teaspoon chili powder",
            "1/2 teaspoon ground turmeric",
            "1/2 teaspoon ground cumin",
            "1/2 teaspoon ground coriander",
            "1/4 teaspoon baking powder",
            "Salt, to taste",
            "Water, as needed",
            "Oil, for frying"
        ],
        "instructions": [
            "Prepare Batter: In a mixing bowl, combine chickpea flour, chili powder, ground turmeric, ground cumin, ground coriander, baking powder, and salt. Gradually add water while whisking to form a smooth batter.",
            "Coat Potatoes: Dip potato wedges into the chickpea flour batter, ensuring they are evenly coated.",
            "Fry Viazi Karai: Heat oil in a deep fryer or heavy-bottomed pot to 180°C (350°F). Fry the coated potato wedges in batches until golden brown and crispy, about 4-5 minutes per batch. Drain on paper towels.",
            "Serve: Serve hot viazi karai as a spicy and satisfying snack or side dish. Enjoy!"
        ],
                "cook_time": "About 4-5 minutes per batch for frying the viazi karai."
    },
    {
        "name": "Kuku",
        "description": "A creamy and aromatic Kenyan chicken curry cooked in coconut milk and flavored with spices.",
        "ingredients": [
            "1 whole chicken, cut into pieces",
            "2 tablespoons vegetable oil",
            "1 onion, finely chopped",
            "3 tomatoes, chopped",
            "1 cup coconut milk",
            "2 tablespoons tomato paste",
            "2 cloves garlic, minced",
            "1 tablespoon ginger, minced",
            "2 green chilies, chopped",
            "1 teaspoon ground coriander",
            "1 teaspoon ground cumin",
            "1/2 teaspoon ground turmeric",
            "1/2 teaspoon paprika",
            "Salt and black pepper, to taste",
            "Fresh cilantro, for garnish"
        ],
        "instructions": [
            "Sauté Aromatics: In a large pot, heat vegetable oil over medium heat. Add finely chopped onion and sauté until translucent. Stir in minced garlic, minced ginger, and chopped green chilies. Cook until fragrant.",
            "Add Tomatoes: Stir in chopped tomatoes and cook until softened.",
            "Cook Chicken: Add chicken pieces to the pot and cook until browned on all sides.",
            "Add Spices: Sprinkle ground coriander, ground cumin, ground turmeric, paprika, salt, and black pepper over the chicken. Stir to coat evenly.",
            "Simmer: Pour coconut milk and tomato paste into the pot. Stir to combine. Cover and let the chicken curry simmer for 30-40 minutes, or until the chicken is cooked through and the sauce has thickened.",
            "Serve: Garnish kuku paka with fresh cilantro before serving. Enjoy this creamy and aromatic chicken curry with rice or naan!"
        ],
        "cook_time": "Approximately 30-40 minutes for simmering the chicken curry."
    },
    {
        "name": "Classic Spaghetti Carbonara",
        "description": "This classic Italian pasta dish features a creamy sauce made with eggs, cheese, pancetta, and black pepper.",
        "ingredients": [
            
                "350g spaghetti",
                "150g pancetta or guanciale, diced",
                "2 large eggs",
                "50g grated Pecorino Romano cheese",
                "50g grated Parmesan cheese",
                "Freshly ground black pepper",
                "Salt, to taste",
                "Fresh parsley, chopped (for garnish)"
                    
                "Garlic cloves, minced",
                "Red pepper flakes, for heat"
        
        ],
        "instructions": [
            "Prepare the Spaghetti: Cook the spaghetti in a large pot of salted boiling water until al dente. Reserve 1 cup of pasta water, then drain the spaghetti.",
            "Cook the Pancetta: Heat a large skillet over medium heat. Add the diced pancetta and cook until crispy and golden brown, about 5-7 minutes. Remove from heat.",
            "Prepare the Sauce: In a mixing bowl, whisk together the eggs, grated Pecorino Romano cheese, and grated Parmesan cheese until well combined. Season with black pepper.",
            "Combine Everything: Add the cooked spaghetti to the skillet with the pancetta. Toss to combine, then pour the egg and cheese mixture over the hot spaghetti. Toss quickly to coat.",
            "Serve: Divide the spaghetti carbonara among serving plates or bowls. Garnish with chopped parsley and extra black pepper, if desired. Serve immediately."
        ],
        "cook_time": "No specific cook time given, but estimated to be about 20-25 minutes for preparing and cooking the dish."    
    },
    {
        "name": "Chicken Alfredo Pasta",
        "description": "A comforting and creamy pasta dish featuring tender chicken and a rich Alfredo sauce.",
        "ingredients": [
        
                "300g fettuccine pasta",
                "2 boneless, skinless chicken breasts, thinly sliced",
                "2 cups heavy cream",
                "1 cup grated Parmesan cheese",
                "4 cloves garlic, minced",
                "2 tablespoons butter",
                "Salt and black pepper, to taste",
                "Fresh parsley, chopped (for garnish)"
                    
                "Broccoli florets, steamed",
                "Cherry tomatoes, halved",
                "Crushed red pepper flakes, for heat"
            
        ],
        "instructions": [
            "Cook the Pasta: Cook the fettuccine pasta in a large pot of salted boiling water until al dente. Drain and set aside.",
            "Cook the Chicken: In a large skillet, melt the butter over medium-high heat. Add the sliced chicken breasts and cook until golden brown and cooked through, about 6-8 minutes. Remove from skillet and set aside.",
            "Prepare the Sauce: In the same skillet, add minced garlic and cook until fragrant, about 1 minute. Pour in the heavy cream and bring to a simmer. Stir in the grated Parmesan cheese until melted and smooth. Season with salt and black pepper to taste.",
            "Combine Everything: Add the cooked pasta and cooked chicken to the skillet with the Alfredo sauce. Toss everything together until well coated and heated through.",
            "Serve: Divide the chicken Alfredo pasta among serving plates. Garnish with chopped parsley and additional grated Parmesan cheese, if desired. Serve hot."
        ],
        "cook_time": "Approximately 20-25 minutes for preparing and cooking the dish."
    },
    {
        "name": "Classic Beef Lasagna",
        "description": "Layers of tender pasta, savory beef sauce, and creamy cheese make this classic lasagna a family favorite.",
        "ingredients": [
            
                "9 lasagna noodles",
                "500g ground beef",
                "1 onion, finely chopped",
                "3 cloves garlic, minced",
                "800g canned crushed tomatoes",
                "2 tablespoons tomato paste",
                "1 teaspoon dried basil",
                "1 teaspoon dried oregano",
                "Salt and black pepper, to taste",
                "250g ricotta cheese",
                "1 egg",
                "1 cup shredded mozzarella cheese",
                "1/2 cup grated Parmesan cheese",
                "Fresh parsley, chopped (for garnish)"
                    
                "Sliced mushrooms",
                "Chopped spinach",
                "Red wine, for deglazing"
            
        ],
        "instructions": [
            "Cook the Lasagna Noodles: Cook the lasagna noodles according to package instructions until al dente. Drain and set aside.",
            "Prepare the Beef Sauce: In a large skillet, cook the ground beef, chopped onion, and minced garlic over medium heat until the beef is browned and the onion is softened. Drain excess fat if needed. Stir in the crushed tomatoes, tomato paste, dried basil, dried oregano, salt, and black pepper. Simmer for 15-20 minutes, stirring occasionally.",
            "Prepare the Cheese Mixture: In a mixing bowl, combine the ricotta cheese, egg, and grated Parmesan cheese. Season with salt and black pepper to taste.",
            "Assemble the Lasagna: Preheat the oven to 180°C (350°F). Spread a thin layer of the beef sauce in the bottom of a 9x13-inch baking dish. Place 3 lasagna noodles on top. Spread a layer of the ricotta cheese mixture over the noodles, followed by a layer of beef sauce. Repeat the layers until all ingredients are used, ending with a layer of beef sauce on top. Sprinkle shredded mozzarella cheese over the top.",
            "Bake the Lasagna: Cover the baking dish with aluminum foil and bake in the preheated oven for 30 minutes. Remove the foil and bake for an additional 15 minutes, or until the cheese is bubbly and golden brown.",
            "Serve: Let the lasagna cool for a few minutes before slicing. Garnish with chopped parsley and serve hot."
        ],
        "cook_time": "Approximately 1 hour and 15 minutes for preparing and baking the lasagna."
    },
    {
        "name": "Vegetable Stir-Fry",
        "description": "A quick and healthy stir-fry packed with colorful vegetables and savory flavors.",
        "ingredients": [
            
                "2 cups mixed vegetables (such as bell peppers, broccoli, carrots, snap peas)",
                "200g firm tofu, drained and cubed",
                "3 tablespoons soy sauce",
                "2 tablespoons sesame oil",
                "2 cloves garlic, minced",
                "1 tablespoon ginger, minced",
                "1 tablespoon cornstarch",
                "1 tablespoon water",
                "Cooked rice, for serving"
                    
                "Sliced mushrooms",
                "Baby corn",
                "Water chestnuts",
                "Chopped scallions, for garnish"
            
        ],
        "instructions": [
            "Prepare the Sauce: In a small bowl, whisk together soy sauce, sesame oil, minced garlic, and minced ginger.",
            "Cook the Tofu: In a large skillet or wok, heat 1 tablespoon of sesame oil over medium-high heat. Add cubed tofu and cook until golden brown on all sides. Remove tofu from skillet and set aside.",
            "Stir-Fry Vegetables: In the same skillet or wok, add remaining sesame oil. Add mixed vegetables and stir-fry for 3-4 minutes until slightly tender but still crisp.",
            "Combine Sauce: Return cooked tofu to the skillet with the vegetables. Pour the prepared sauce over the tofu and vegetables, tossing to coat evenly.",
            "Thicken Sauce: In a small bowl, mix cornstarch and water to make a slurry. Add the slurry to the skillet and cook for an additional 1-2 minutes until the sauce thickens.",
            "Serve: Serve the vegetable stir-fry over cooked rice. Garnish with chopped scallions, if desired. Enjoy!"
        ],
        "cook_time": "Approximately 20 minutes"
    },
    {
        "name": "Mushroom Risotto",
        "description": "A creamy and flavorful risotto made with Arborio rice and sautéed mushrooms.",
        "ingredients": [
            
                "1 cup Arborio rice",
                "4 cups vegetable or chicken broth",
                "200g mushrooms (such as cremini or shiitake), sliced",
                "1 onion, finely chopped",
                "2 cloves garlic, minced",
                "1/2 cup dry white wine",
                "1/2 cup grated Parmesan cheese",
                "2 tablespoons butter",
                "2 tablespoons olive oil",
                "Salt and black pepper, to taste",
                "Fresh parsley, chopped (for garnish)"
                    
                "1/2 cup heavy cream",
                "Truffle oil, for drizzling",
                "Fresh thyme leaves, for garnish"
            
        ],
        "instructions": [
            "Prepare the Broth: In a saucepan, heat the vegetable or chicken broth over medium heat until simmering. Keep warm while preparing the risotto.",
            "Sauté Mushrooms: In a large skillet or Dutch oven, heat olive oil over medium heat. Add sliced mushrooms and cook until they release their moisture and become golden brown, about 6-8 minutes. Remove mushrooms from skillet and set aside.",
            "Sauté Onion and Garlic: In the same skillet, melt butter over medium heat. Add chopped onion and minced garlic, cooking until softened and fragrant, about 3-4 minutes.",
            "Cook Rice: Add Arborio rice to the skillet with the sautéed onion and garlic. Stir to coat the rice with butter and cook for 1-2 minutes until slightly translucent.",
            "Deglaze with Wine: Pour dry white wine into the skillet and cook, stirring constantly, until the wine is absorbed by the rice.",
            "Add Broth: Begin adding warm broth to the skillet, one ladleful at a time, stirring frequently and allowing each addition to be absorbed before adding more. Continue until the rice is creamy and al dente, about 20-25 minutes.",
"Finish Risotto: Stir in the cooked mushrooms, grated Parmesan cheese, anvy cream, if using. Season with salt and black pepper to taste.",
            "Serve: Divide the mushroom risotto among serving plates. Garnish with chopped parsley and fresh thyme leaves. Drizzle with truffle oil, if desired. Serve hot."
        ],
        "cook_time": "Approximately 40-45 minutes"
    },
    {
        "name": "Chicken Tikka Masala",
        "description": "A classic Indian dish featuring tender marinated chicken in a creamy tomato-based sauce.",
        "ingredients": [
            
                "500g boneless, skinless chicken thighs, cut into bite-sized pieces",
                "1 cup plain yogurt",
                "4 cloves garlic, minced",
                "1 tablespoon ginger, minced",
                "2 tablespoons lemon juice",
                "2 teaspoons ground cumin",
                "2 teaspoons ground coriander",
                "2 teaspoons paprika",
                "1 teaspoon turmeric",
                "1/2 teaspoon cayenne pepper",
                "Salt and black pepper, to taste",
                "2 tablespoons ghee or butter",
                "1 onion, finely chopped",
                "1 can (400g) crushed tomatoes",
                "1 cup heavy cream",
                "Fresh cilantro, chopped (for garnish)",
                "Cooked basmati rice, for serving",
                "Naan bread, for serving"
                    
                "Garam masala",
                "Ground fenugreek",
                "Chopped green chilies, for extra heat"
            
        ],
        "instructions": [
            "Marinate Chicken: In a mixing bowl, combine plain yogurt, minced garlic, minced ginger, lemon juice, ground cumin, ground coriander, paprika, turmeric, cayenne pepper, salt, and black pepper. Add chicken pieces to the marinade, coating evenly. Cover and refrigerate for at least 1 hour or overnight.",
            "Cook Chicken: In a large skillet or Dutch oven, heat ghee or butter over medium heat. Add marinated chicken pieces and cook until browned and cooked through, about 8-10 minutes. Remove chicken from skillet and set aside.",
            "Prepare Sauce: In the same skillet, add chopped onion and cook until softened and translucent, about 5 minutes. Stir in crushed tomatoes and cook for an additional 5 minutes. Add heavy cream and cooked chicken to the skillet, stirring to combine.",
            "Simmer Sauce: Bring the mixture to a simmer and let it cook for 10-15 minutes, allowing the flavors to meld and the sauce to thicken. Season with additional salt and black pepper if needed.",
            "Serve: Garnish chicken tikka masala with chopped fresh cilantro. Serve hot over cooked basmati rice with naan bread on the side."
        ],
        "cook_time": "Approximately 30-35 minutes (plus marinating time of at least 1 hour)"
    },
    {
        "name": "Beef Tacos",
        "description": "A crowd-pleasing Mexican favorite featuring seasoned ground beef, fresh toppings, and warm tortillas.",
        "ingredients": [
            
                "500g ground beef",
                "1 onion, finely chopped",
                "2 cloves garlic, minced",
                "1 tablespoon chili powder",
                "1 teaspoon ground cumin",
                "1/2 teaspoon paprika",
                "1/2 teaspoon dried oregano",
                "Salt and black pepper, to taste",
                "8 small corn or flour tortillas",
                "Shredded lettuce",
                "Diced tomatoes",
                "Shredded cheddar cheese",
                "Sliced jalapeños",
                "Sour cream",
                "Fresh cilantro, chopped (for garnish)"
                    
                "Sliced avocado or guacamole",
                "Salsa",
                "Hot sauce"
            
        ],
        "instructions": [
            "Cook Beef: In a large skillet, cook ground beef over medium heat until browned and cooked through, breaking it apart with a spoon. Drain excess fat if needed.",
            "Season Beef: Add chopped onion, minced garlic, chili powder, ground cumin, paprika, dried oregano, salt, and black pepper to the skillet with the cooked beef. Stir to combine and cook for an additional 2-3 minutes.",
            "Warm Tortillas: Heat tortillas in a dry skillet or microwave until warm and pliable.",
            "Assemble Tacos: Spoon seasoned ground beef onto warm tortillas. Top with shredded lettuce, diced tomatoes, shredded cheddar cheese, sliced jalapeñosour cream, anpings as desired.",
            "Serve: Garnish beef tacos with chopped fresh cilantro. Serve immediately with additional toppings on the side."
        ],
        "cook_time": "Approximately 20 minutes"
    },
    {
        "name": "Vegetable Curry",
        "description": "A hearty and flavorful vegetarian curry featuring an array of colorful vegetables in a fragrant sauce.",
        "ingredients": [
            
                "2 tablespoons vegetable oil",
                "1 onion, finely chopped",
                "3 cloves garlic, minced",
                "1 tablespoon ginger, minced",
                "2 tablespoons curry powder",
                "1 teaspoon ground cumin",
                "1 teaspoon ground coriander",
                "1/2 teaspoon turmeric",
                "1/2 teaspoon paprika",
                "1 can (400g) coconut milk",
                "2 cups vegetable broth",
                "2 cups mixed vegetables (such as potatoes, carrots, bell peppers, peas)",
                "Salt and black pepper, to taste",
                "Cooked rice, for serving",
                "Fresh cilantro, chopped (for garnish)"
                    
                "Chopped green chilies, for heat",
                "Chickpeas or lentils, for added protein"
            
        ],
        "instructions": [
            "Sauté Aromatics: In a large skillet or Dutch oven, heat vegetable oil over medium heat. Add chopped onion and cook until softened and translucent, about 5 minutes. Stir in minced garlic and minced ginger, cooking for an additional 2 minutes until fragrant.",
            "Add Spices: Sprinkle curry powder, ground cumin, ground coriander, turmeric, and paprika over the sautéed aromatics. Stir to coat the onion mixture with the spices.",
            "Prepare Sauce: Pour coconut milk and vegetable broth into the skillet with the spiced onion mixture. Bring the mixture to a simmer.",
            "Cook Vegetables: Add mixed vegetables to the skillet, stirring to combine. Simmer for 15-20 minutes, or until the vegetables are tender and the sauce has thickened.",
            "Season: Season the vegetable curry with salt and black pepper to taste. Adjust seasoning as needed.",
            "Serve: Serve the vegetable curry over cooked rice. Garnish with chopped fresh cilantro. Enjoy!"
        ],
        "cook_time": "Approximately 30-35 minutes"
    },
    # Add more recipes...
]
